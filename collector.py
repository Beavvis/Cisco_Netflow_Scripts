...
"""
import os                               # added for cyclical folders & files
import argparse
import gzip
import json
import logging
...
import threading
import time
from datetime import datetime           # added for cyclical folders & files
...
if __name__ == "__main__":
    ...
    try:
        ...
        for ts, client, export in get_export_packets(args.host, args.port):
            ...
            # =====================================================================================================================
            # START - modifications for cyclical file & folder rotational logs
            # =====================================================================================================================
            
            home = "/local/netflow_data"                            # <= Rename the folder here to where you want your NETFLOW data files
            date = current_time.strftime("%Y-%m-%d")
            hour = current_time.strftime("%H")
            folder_str = f"{home}{date}_netflows/"
            os.makedirs(folder_str, exist_ok=True)
            timestamp_str = curent_time.strftime("%Y-%m-%d")        # Timestamp Format Example: 2025-12-15
            filename = f"{folder_str}/netflow_{hour}:00.gz"         # Folder name Format Example: 2025-12-15_12:00_netflow.gz
  
            #with gzip.open(args.output_file, "ab") as fh:  # open as append, not reading the whole file  REMARK: THIS IS THE ORIGINAL LINE
            
            with gzip.open(filename, "ab") as fh:  # open as append, not reading the whole file
                fh.write(line)
                
            # =====================================================================================================================
            # END - modifications for cyclical file & folder rotational logs
            # =====================================================================================================================
